...
export default function CustomerLoginPage() {
  return <Suspense fallback={<main className="min-h-screen bg-stone-50" />}><CustomerLoginContent /></Suspense>;
}
    ...
    <main className="login-shell">
      <section className="brand-panel" aria-label="About Trimly">
        ...
        <div className="brand-copy">
          ...
          <p className="brand-description">Log in as a customer to manage your bookings and account.</p>
        </div>
      </section>
      ...
      <section className="form-panel">
        <div className="login-card">
          ...
          <div className="form-heading">
            ...
            <h2>Log in to Trimly</h2>
            <p>Use your customer password. Your owner password can be different.</p>
          </div>
          <form onSubmit={submit} noValidate aria-busy={isSubmitting}>
            ...
            <div className="field-group">
              ...
              <div className="password-field">
                <input id="password" name="password" type={showPassword ? 'text' : 'password'} autoComplete="current-password" placeholder="Enter your customer password" />
            ...
            <button className="submit-button" type="submit" disabled={isSubmitting}>
              {isSubmitting ? 'Logging in…' : 'Log In as Customer'}
              {!isSubmitting && <ArrowRight />}
            </button>
            {error && <p className="form-status form-status-error" role="alert">{error}</p>}
          </form>
          <p className="signup-copy">New customer? <Link href="/signup">Create an account</Link></p>
          <p className="signup-copy">Manage a shop? <Link href="/owner/login">Owner login</Link></p>
          <p className="signup-copy">Own a new shop? <Link href="/owner/register">Register your business</Link></p>
        </div>
      </section>
    </main>
  );
